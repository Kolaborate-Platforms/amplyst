...
const CompletionStep = ({ data, onComplete }: CompletionStepProps) => {
  ...
  const handleComplete = async () => {
    try {
      ...
      // Prepare the data for the insertProfile mutation
      const profileData = {
        role: data.role as "influencer" | "brand" | "agency",
      ...
      // Navigate to dashboard if router is ready
      if (isRouterReady) {
  ...
  console.log("actualFollowerCount:", actualFollowerCount);
  
  const followerSource = getFollowerCountSource();
  console.log("followerSource:", followerSource);
    ...
    <div className="space-y-6 animate-fade-in text-center">
      ...
      <div className="pt-4">
        <Button
          ...
          disabled={isLoading || (!isRouterReady && !onComplete)}
        ...
        >
          {isLoading ? 'Saving Profile...' : (!isRouterReady && !onComplete) ? 'Loading...' : 'Complete Setup & Go to Dashboard'}
        </Button>
      </div>
