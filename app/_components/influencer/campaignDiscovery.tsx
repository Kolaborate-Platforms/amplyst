...
interface Profile {
  ...
  location?: string;
  role?: string;
  followerCount?: string | number; // Allow both string and number
  portfolio?: Array<{
    type: string;
    title?: string;
    description?: string;
...
const CampaignDiscovery = ({ campaigns, profile }: CampaignDiscoveryProps) => {
  ...
  const hasApplied = (campaignId: Id<"campaigns">) => {
    // You'll need to pass applications as a prop or fetch them here
  ...
  const handleViewCampaignDetails = (campaignId: Id<"campaigns">) => {
    router.push(`/dashboard/influencer/discover/campaigns/${campaignId}`);
  };

  // Helper function to get follower count as number
  const getFollowerCount = (profile: Profile): number => {
    // Try to get from portfolio first
    const portfolioFollowers = profile.portfolio?.find(item => item.type === 'image')?.metrics?.followers;
    if (portfolioFollowers) {
      return typeof portfolioFollowers === 'string' ? parseInt(portfolioFollowers) || 0 : portfolioFollowers;
    }
    
    // Fallback to followerCount field
    if (profile.followerCount) {
      return typeof profile.followerCount === 'string' ? parseInt(profile.followerCount) || 0 : profile.followerCount;
    }
